...
Applications that keep credentials in a database can use `Identity::to_bytes`/`from_bytes` and `generate_host_state()` to seed their private storage without binding a transport. Implement `HostStorage` and pass it to `MessagingEndpoint::host_with_storage` to persist enrollment updates. Its synchronous `save` callback must atomically commit the complete opaque state before returning success; keep both the identity bytes and state confidential. Failed saves roll back the in-memory grant or enrollment change. `load` must return existing state on restart; returning `None` creates a new authority.
